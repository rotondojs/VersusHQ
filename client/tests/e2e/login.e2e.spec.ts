...
/**
 * Wait for the login page to finish initialising before interacting.
 */
...
test.describe("The login page", () => {
  // GoogleOAuthProvider loads https://accounts.google.com/gsi/client on every
  // page load.  In CI the request can take 10–15 s, and when it finally
  // resolves it triggers a React re-render that briefly removes DOM nodes.
  // Aborting the request keeps `scriptLoadedSuccessfully` at its initial
  // `false` value (no state change → no re-render), so the login form is
  ...
  test("should allow an existing user to log in", async ({ page }) => {
    ...
    await expect(page.getByText("Frau Drei")).toBeVisible();
  });
  ...
  test("should reject creating an account for an existing user, and allow correction", async ({
    ...
    await page.getByRole("button", { name: "Sign Up" }).click();
    await page.waitForURL(`/profile-setup/${randUsername}`);
  });
});
