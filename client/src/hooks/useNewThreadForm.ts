...
export default function useNewThreadForm() {
  ...
  const createThreadMutation = useMutation({
    mutationFn: async (payload: { title: string; text: string }) => {
      const thread = await createThread(auth, payload);
      if ("error" in thread) {
  ...
  const handleSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      await createThreadMutation.mutateAsync({ title, text: contents });
    } catch (caughtError) {
      setErr(
        caughtError instanceof Error ? caughtError.message : "Unable to create post right now.",
      );
    }
  };
