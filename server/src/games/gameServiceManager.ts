...
export interface GameServicer {
   ...
   * @returns the initial state plus view updates for players and watchers
   */
...
export class GameService<State, View> implements GameServicer {
  ...
  /**
   * Wraps a concrete `GameLogic` implementation in the generic `GameServicer` interface.
   */
  ...
  /**
   * Exposes the minimum player count supported by the underlying game.
   */
  get minPlayers() {
  ...
  /**
   * Exposes the maximum player count supported by the underlying game.
   */
  get maxPlayers() {
  ...
  /**
   * Builds a tagged view of the current state for a specific player or watcher.
   */
  ...
  /**
   * Creates the watcher view and each player-specific view for a state snapshot.
   */
  ...
  /**
   * Starts a new game state and returns the initial view updates.
   */
  ...
  /**
   * Applies a move to the game and packages the resulting view updates.
   */
  ...
  /**
   * Returns the tagged game view for one player or watcher.
   */
  ...
  /**
   * Delegates winner selection to the underlying game logic.
   */
  getWinner(state: any) {
    return this._logic.getWinner(state);
  }
}
