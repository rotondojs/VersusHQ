/**
 * REST and socket handlers for creating, joining, viewing, and updating games.
 */
import {
  type GameInfo,
  withAuth,
...
export const postCreate: RestAPI<GameInfo> = async (req, res) => {
  const body = withAuth(zCreateGameRequest).safeParse(req.body);
  if (body.error) {
...
/**
 * Writes a move-log entry into the game's chat and broadcasts it to the chat room.
 */
async function broadcastMoveLog(
  io: GameServer,
  chatId: string,
  moveDescription: string,
  actor: Parameters<typeof addMoveLogToChat>[2],
) {
  const payload = await addMoveLogToChat(chatId, moveDescription, actor, new Date());
  io.to(chatId).emit("chatMoveLog", payload);
}
...
export const socketMakeMove: SocketAPI = (socket, io) => async (body) => {
  try {
    ...
    await broadcastMoveLog(io, chatId, moveDescription, user);

    let automatedMove = await runBattleshipAiTurn(gameId);
    while (automatedMove) {
      sendViewUpdates(io, gameId, automatedMove.views);
      await broadcastMoveLog(
        io,
        automatedMove.chatId,
        automatedMove.moveDescription,
        automatedMove.actor,
      );
      automatedMove = await runBattleshipAiTurn(gameId);
    }
  } catch (err) {
    logSocketError(socket, err);
  }
